...
#define MPU_ADDRESS		0x68 // AD0 pin na 3.3 V in pol je 0x69
...
void MPUtestConnection()
{
	...
	if(WhoAmI == 0x68)
	{
		UARTprintf("Connection succesful %d ! \n", MPU_ADDRESS);
		GreenLed(true);
		Delay(100);
		GreenLed(false);
	}
...
void readMPU()
{
	...
	int ab = -3;
	UARTprintf("AccXout: %d AccYout: %d AccZout: %d gyroXout: %d gyroYout: %d gyroZout: %d test : %d\n", (int)accXout, (int)accYout, (int)accZout, (int)gyroXout, (int)gyroYout, (int)gyroZout, (int)ab);
...
float getGyroX(){
//	UARTprintf(" X: %d", (int)gyroXos[0]);
	return gyroXos[0];
}

float getGyroY(){
//	UARTprintf(" Y: %d", (int)gyroYos[0]);
	return gyroYos[0];
}
